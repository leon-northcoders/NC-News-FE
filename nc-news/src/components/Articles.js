...
function Articles({ articles, match, searchTerm }) {
  ...
  let displayArticles = articles.filter(
    article => (topic ? article.belongs_to.title.toLowerCase() === topic : true)
  );
  if (searchTerm)
        ...
        return (
          <div key={article._id}>
            <Card
              ...
              className="small"
              header={
                <CardTitle className="card-panel light-blue accent-1" image="">
                  {article.title}
                </CardTitle>
              }
            >
                ...
                <Col s={4}>
                  <Chip>
                    <Link
                      ...
                      to={`/topics/${article.belongs_to.title.toLowerCase()}`}
                    >
